...
rownames(refs2)=refs2[,1]
refs2=refs2[match(colnames(samplematrix2),rownames(refs2)),]
refsall=refs2
MyRNAseqDataRNBall=MyRNAseqDataRNB2
samplematrixall=as.matrix(MyRNAseqDataRNB2)
```


RNB3
```{r}
MyRNAseqDataRNB3=read.table("CombinedRNB3.txt")
Mylabels=read.table('listcntRNB3.txt',header=TRUE)
rownames(MyRNAseqDataRNB3)=MyRNAseqDataRNB3[,1]
MyRNAseqDataRNB3=MyRNAseqDataRNB3[,-1]
colnames(MyRNAseqDataRNB3)=Mylabels[,1]
samplematrix3=as.matrix(MyRNAseqDataRNB3)
refs3= read.table("PhenotypeRNB3.txt",header=TRUE)
rownames(refs3)=refs3[,1]

refsall=rbind(refs2,refs3)
MyRNAseqDataRNBall=cbind(MyRNAseqDataRNB2,MyRNAseqDataRNB3)
samplematrixall=as.matrix(MyRNAseqDataRNBall)
```

RNB3.5-4
```{r}
MyRNAseqDataRNB4=read.table("CombinedRNB4.txt")
Mylabels=read.table('listcntRNB3.5-4.txt',header=TRUE)
rownames(MyRNAseqDataRNB4)=MyRNAseqDataRNB4[,1]
MyRNAseqDataRNB4=MyRNAseqDataRNB4[,-1]
colnames(MyRNAseqDataRNB4)=Mylabels[,1]
samplematrix4=as.matrix(MyRNAseqDataRNB4)
refs4= read.table("PhenotypeRNB3.5-4.txt",header=TRUE)
rownames(refs4)=refs4[,1]
refs4=refs4[match(colnames(samplematrix4),rownames(refs4)),]


refsall=rbind(refs2,refs3,refs4)
MyRNAseqDataRNBall=cbind(MyRNAseqDataRNB2,MyRNAseqDataRNB3,MyRNAseqDataRNB4)
samplematrixall=as.matrix(MyRNAseqDataRNBall)
```
...
write.table(restest,"RNB1-4_Test_DEseq2_Unlimited.txt")
# or to shrink log fold changes association with condition:
res <- results(dds, name="Regen_YES_vs_NO")
write.table(res,"RNB1-4_DEseq2_Unlimited.txt")
```

Lets check which direction is positive
```{r}
Firstgene=t(MyRNAseqData[match("ENSMUSG00000006356.10",rownames(MyRNAseqDataRNB3)),])
```


Ok. now we know "YES" is positive. Lets use biomart to convert ensemble id
```{r}
library(biomaRt)
